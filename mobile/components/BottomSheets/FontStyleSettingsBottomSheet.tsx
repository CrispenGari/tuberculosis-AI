...
import { COLORS, FONTS, FONTSTYLES } from "@/constants";
import {
  BottomSheetModal,
  BottomSheetBackdrop,
  BottomSheetView,
...
interface Props {}
const FontStyleSettingsBottomSheet = React.forwardRef<BottomSheetModal, Props>(
  ({}, ref) => {
...
export default FontStyleSettingsBottomSheet;

export const FontStyleComponent = () => {
  const { settings, update } = useSettingsStore();
  const [value, setValue] = React.useState(settings.fontStyle);
  ...
  return (
    <DropdownSelect
      ...
      onValueChange={async (value: any) => {
        ...
        setValue(value);
        update({ ...settings, fontStyle: value });
        dismiss();
      }}
      ...
      helperText="Select the font style that you want to use in this app."
      selectedItemStyle={{
        color: settings.theme === "dark" ? COLORS.white : COLORS.black,
        fontSize: 18,
        fontFamily: FONTS.regular,
